...
def mask_non_prose(text: str) -> str:
    """按源位置屏蔽常见 Markdown 非正文；不把任意尖括号当标签。"""
    ...
    # 前置元数据中标题/摘要/说明会展示，不能一概豁免。
    front = re.match(r"\A\ufeff?---[ \t]*\n.*?\n---[ \t]*(?:\n|\Z)", text, re.DOTALL)
    if front:
        visible_block = False
        offset = 0
        for line in front.group().splitlines(keepends=True):
            field = re.match(r"(title|description|caption)[ \t]*:[ \t]*(.*)", line)
            if field:
                hide(offset, offset + field.start(2))
                visible_block = field.group(2).strip() in {"|", ">", "|-", ">-", "|+", ">+"}
            elif not (visible_block and line.startswith((" ", "\t"))):
    ...
    for line in text.splitlines(keepends=True):
        ...
        if fence:
            ...
            # 只在段落边界或已有缩进块内识别，避免吞掉普通段落续行。
            if not previous_line.strip() or previous_line.startswith(("    ", "\t")):
                hide(offset, offset + len(line))
        offset += len(line)
        previous_line = line
    masked = "".join(chars)
    # 同长度反引号才闭合 code span，允许内部包含更短的反引号。
    ...
    while index < len(runs):
        ...
        if end_index is not None:
            hide(runs[index].start(), runs[end_index].end())
            index = end_index + 1
        else:
            index += 1
    patterns = (
        ...
        re.compile(r"</?[A-Za-z][A-Za-z0-9-]*(?:\s+[A-Za-z_:][^>\n]*)?\s*/?>"),
    )
    masked = "".join(chars)
...
def metaphor_cluster(text: str, distance: int = 800):
    ...
    for index, (start, _, _) in enumerate(hits):
        while right < len(hits) and hits[right][0] - start <= distance:
            counts[hits[right][1]] += 1
            right += 1
        if len(counts) >= 3:
            return hits[index:right], set(counts)
        counts[hits[index][1]] -= 1
        if counts[hits[index][1]] == 0:
            del counts[hits[index][1]]
    return None
...
def read_text(path: str) -> str:
    if path == "-":
        value = sys.stdin.read(2_000_001)
    else:
        if Path(path).stat().st_size > 8_000_000:
            raise ValueError("稿件超过 8 MB，请分章检查")
        with Path(path).open(encoding="utf-8-sig") as handle:
            value = handle.read(2_000_001)
    if len(value) > 2_000_000:
        raise ValueError("稿件超过 200 万字符，请分章检查")
    return value.lstrip("\ufeff")
...
def main() -> int:
    ...
    parser.add_argument("--preserve-line", action="append", default=[], metavar="N=理由", help="保留已人工确认的原话/对白/术语或用户指定行；可重复，不允许省略理由")
    ...
    preserved = []
    for specification in args.preserve_line:
        number, separator, reason = specification.partition("=")
        if not separator or not number.isdecimal() or not reason.strip() or not 1 <= int(number) <= len(lines):
            print("--preserve-line 必须是有效行号=保留理由。", file=sys.stderr)
            return 2
        index = int(number) - 1
        lines[index] = "".join("\n" if c == "\n" else " " for c in lines[index])
        preserved.append((int(number), reason.strip()))
    prose = "".join(lines)
    total_han = han_count(prose)
    if total_han == 0 and not preserved:
    ...
    for match in road_signs:
        warnings.append(
            f"待辨语境路标，第 {line_number(text, match.start())} 行，"
    ...
    if total_han >= 600 and len(conjunction_hits) * 1000 / total_han > 7:
        ...
        warnings.append(
            ...
            "逐个检查是否承担逻辑；删后指向或因果受损就保留，不按比例删除。"
        )
    ...
    if cv_result and cv_result[0] < 0.42:
        warnings.append(
            ...
            "只检查停顿是否合意；自然的整齐节奏可以保留，不为改变统计指标改句。"
        )
    ...
    for number, reason in preserved:
        print(f"已人工保留第 {number} 行（整行未检查），理由：{reason}")
    print(
    ...
    if not failures and not warnings:
        print("\n未发现这份检查器覆盖的问题。")
    if len(failures) > 100 or len(warnings) > 100:
        print("仅显示每类前 100 项；请修改或分章后复查。")
    print("退出码 0 仅表示没有机器硬错误，不代表事实、风格或活人感已通过。")
